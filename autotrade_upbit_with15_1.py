...
def get_ma15(ticker):
    ...
    df['ma15'] = df['low'].rolling(15).mean()
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=300):
            ...
            df = get_ma15("KRW-DOGE")
            # if target_price < current_price: 
            if target_price < current_price and df.iloc[-1]['ma15'] < current_price and \
                df.iloc[-5]['ma15'] < df.iloc[-3]['ma15'] < df.iloc[-1]['ma15']:
                krw = get_balance("KRW")
                if krw > 5000:
            ...
            # 따라서 매도는 당일 결정가(당일 변동폭의 50%) 기준 및 전일 저가 기준으로 결정
            ...
            # log 남기기(현재시간, 현재가, 고가, 저가, K값, 기준가, 전일저가, no sell 기록 남기기)
